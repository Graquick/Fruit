    ...
    <>
      <main className="min-h-screen bg-neutral-700">
        <div className={`w-full h-screen flex items-center justify-center top-0 left-0 right-0 bottom-0 m-auto`}>
          <div className={`w-40 h-[100px] bg-white/40 flex rounded-[50px] p-[10px] cursor-pointer ${open ? 'justify-end bg-green-500' : 'justify-start'}`} onClick={() => setOpen(!open)} variants={toggleVariants} animate={open ? 'opened' : 'closed'}>
