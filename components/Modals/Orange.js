...
export default function Orange({orange, setOrange}) {
    return (
        <AnimatePresence>
            {orange && (
            ...
            >
                <motion.div className={`bg-white shadow-md w-[200px] h-[286px] flex items-center justify-center text-[100px] cursor-pointer`}
                    ...
                    transition={spring}
                >🍊</motion.div>
                <motion.div className={`relative bottom-40 bg-white text-4xl rounded-full cursor-pointer`} whileTap={{ scale: 0.5 }} onClick={() => setOrange(false)}>< Icon icon="ant-design:close-circle-outlined"/></motion.div>
            </motion.div>
            )}
        </AnimatePresence>
    )
